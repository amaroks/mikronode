...
module.exports = (function() {
	...
	Channel.prototype.write = function write(d, parameters, writeCallback) {
		...
		if (d) {
			if (typeof (d) === 'string') {
				d = d.split("\n");
				d = d.filter((item) => { // fixes situation when empty newline is sent to mikrotik when command ends with newline character. Then .tag wouldn't be sent and everything'd be burned.
					return item.trim() != "";
				})
			}
