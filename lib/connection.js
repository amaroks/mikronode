	...
	Connection.prototype.sentence = function sentence(data, more) {
		...
		if (_(this).currentReply === '!fatal') { // our last message was a fatal error.
			...
			if (_(this).currentProgress === '!trap') {
				...
				// Dont know how to restore state (channels are freezed, closing of connection is not enought)
				// Because we dont know .tag value, so we dont know channel (mikrotik will not send)
				// -> force close all of them to prevent connection freezing
				//for (var chan in _(this).channels) { // TODO: fix that traps will be ignored because of channel closing
				//	_(this).channels[chan].close(true);
				//}
			}
